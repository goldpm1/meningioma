...
MUTATIONTIMER_PATH="/home/goldpm1/Meningioma/31.Clonality/03.mutationtimeR"
MUTECT_VCF_DIR="/home/goldpm1/Meningioma/04.mutect/04.rescue/"
SEQUENZA_DIR="/home/goldpm1/Meningioma/11.cnv/2.sequenza"
FACET_CNV_DIR="/home/goldpm1/Meningioma/31.Clonality/01.make_matrix"

MUTATIONTIMER_INPUT_VCF_DIR=${MUTATIONTIMER_PATH}"/01.vcf"
...
for idx in ${!sample_name_LIST[@]}; do
    ...
    for TISSUE in Dura Tumor; do
        ...
        bcftools view -s ${Sample_ID}"_"${TISSUE} ${MUTECT_VCF_PATH} > ${MUTATIONTIMER_INPUT_VCF_PATH}".temp"
        awk '{gsub(/^chr/,""); print}' ${MUTATIONTIMER_INPUT_VCF_PATH}".temp" > ${MUTATIONTIMER_INPUT_VCF_PATH}
        #sed -i "s/ID=chr/ID=/g" ${MUTATIONTIMER_INPUT_VCF_PATH}        # contig에서 chr1 -> 1 로 변형해주기
        rm -rf ${MUTATIONTIMER_INPUT_VCF_PATH}".temp"
        ...
        MUTATIONTIMER_RESUMUTATIONTIMER_RESULT_DIRDIR=${MUTATIONTIMER_PATH}"/02.result_sequenza"
        ...
        FACET_CNV_MATRIX_PATH="/home/goldpm1/Meningioma/31.Clonality/01.make_matrix/"${Sample_ID}"/"${Sample_ID}"_"${TISSUE}".facetcnv_to_bed_df.tsv"
        MUTATIONTIMER_RESULT_DIR=${MUTATIONTIMER_PATH}"/02.result_facetcnv"
        if [ ! -d ${MUTATIONTIMER_RESULT_DIR} ] ; then
            mkdir -p ${MUTATIONTIMER_RESULT_DIR}
        fi
