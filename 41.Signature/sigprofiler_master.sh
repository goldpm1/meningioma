...
#01. Sample 종류별로 모으기 
PASS_VCF_DIR=${PROJECT_DIR}"/04.mutect/02.PASS"

for Sample_ID in 220930 220930_2 221026 221102 221202 230127 230303 230323_2 230405_2 230419 230526 230822 230920 230802_1 230802_2 230802_3 231006 240403_1 240403_2 240403_3 240412_1 240412_2 240412_3; do
    TUMOR_VCF_PATH=${PASS_VCF_DIR}"/"${Sample_ID}"_Tumor.MT2.FMC.HF.RMBLACK.vcf"
    cp ${TUMOR_VCF_PATH} ${SIGPROFILER_INPUT_VCF_DIR}"/Tumor"

    for TISSUE in Dura Ventricle Cortex; do
        DURA_VCF_PATH=${PASS_VCF_DIR}"/"${Sample_ID}"_"${TISSUE}".MT2.FMC.HF.RMBLACK.vcf"
        if [ -f ${DURA_VCF_PATH} ]; then     # File이 있어야만 진행
            cp ${DURA_VCF_PATH} ${SIGPROFILER_INPUT_VCF_DIR}"/Dura"
        fi
    done
done
...
# #01. Sample 종류별로 모으기 
# for Sample_ID in 220930 221026 221102 221202 230127 230303 230323_2 230405_2 230419 230526 230822 230920 230802_1 230802_2 230802_3 231006 240403_1 240403_2 240403_3 240403_1 240403_2 240403_3 ; do
