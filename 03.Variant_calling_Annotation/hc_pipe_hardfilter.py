...
parser.add_argument('--OUTPUT_VCF', type=str, default="")
parser.add_argument('--SAMPLE_THRESHOLD', type=str, default="Blood")
parser.add_argument('--DP_THRESHOLD', type=int, default=100)
parser.add_argument('--REF_THRESHOLD', type=int, default=5)
parser.add_argument('--ALT_THRESHOLD', type=int, default=5)
parser.add_argument('--REMOVE_MULTIALLELIC', type=str, default=True)
parser.add_argument('--PASS', type=str, default=True)
parser.add_argument('--REMOVE_MITOCHONDRIAL_DNA', type=str, default=True)
...
kwargs["DP_THRESHOLD"] = int(args.DP_THRESHOLD)
kwargs["REF_THRESHOLD"] = int(args.REF_THRESHOLD)
...
if "gz" in kwargs["INPUT_VCF"]:
    input_file = gzip.open ( kwargs["INPUT_VCF"], "r")
else:
...
for line in input_file.readlines():
    ...
    if kwargs["REMOVE_MITOCHONDRIAL_DNA"] == True:
        if CHR not in ["chr" + str(i) for i in range (1,23) ] + ["chrX, chrY"]:
            check = 1        
    #print ("REMOVE_MITOCHONDRIAL_DNA  : " ,check)
    if kwargs["PASS"] == True:
    ...
    if kwargs["REMOVE_MULTIALLELIC"]  == True:
        if "," in line[4]:
            check = 1
    #print ("REMOVE_MULTIALLEIC : ", check)
    ...
    for i in range (9, len (line)):
        ...
        if check2 == True:
            if int( sample_dict [i - 9] ["DP"] ) < kwargs["DP_THRESHOLD"]:
                ...
                #print (line, sample_dict [i - 9] ["AD"])
                if ( int ( sample_dict [i - 9] ["AD"].split(",")[0] ) < kwargs ["REF_THRESHOLD"] ) | ( int ( sample_dict [i - 9] ["AD"].split(",")[1] ) < kwargs ["ALT_THRESHOLD"] ):
                    check = 1
                    break
