...
# python3 /data/project/Meningioma/script/03.Variant_calling_Annotation/mutect_pair_pipe_04.Other_rescue.py

parser = argparse.ArgumentParser( description='The below is usage direction.')
parser.add_argument('--Sample_ID', type=str, default = "241023")
parser.add_argument('--TISSUE', type=str, default = "Dura")
parser.add_argument('--TUMOR_INTERVAL', type=str, default = "/data/project/Meningioma/04.mutect/03.Tumor_interval/241023_Tumor.MT2.FMC.HF.RMBLACK.bed")
parser.add_argument('--TUMOR_BAM_PATH', type=str, default = "/data/project/Meningioma/02.Align/hg38/Tumor/05.Final_bam/241023_Tumor.bam")
parser.add_argument('--RESCUE_BAM_PATH', type=str, default = "/data/project/Meningioma/02.Align/hg38/Dura/05.Final_bam/241023_Dura.bam")
parser.add_argument('--BLOOD_BAM_PATH', type=str, default = "/data/project/Meningioma/02.Align/hg38/Blood/05.Final_bam/241023_Blood.bam")
parser.add_argument('--TUMOR_MUTECT2_VCF', type=str, default = "/data/project/Meningioma/04.mutect/02.PASS/241023_Tumor.MT2.FMC.HF.RMBLACK.vcf")
parser.add_argument('--OTHER_MUTECT2_VCF', type=str, default = "/data/project/Meningioma/04.mutect/02.PASS/241023_Dura.MT2.FMC.HF.RMBLACK.vcf")
parser.add_argument('--RESCUE_VCF', type=str, default = "/data/project/Meningioma/04.mutect/04.Other_rescue/241023_Dura.MT2.FMC.HF.RMBLACK.rescue.vcf")
parser.add_argument('--TUMOR_SHARED_VARIANT_VCF', type=str, default = "/data/project/Meningioma/04.mutect/05.Shared_variant/241023_Tumor.MT2.FMC.HF.RMBLACK.shared_variant.vcf")
parser.add_argument('--OTHER_SHARED_VARIANT_VCF', type=str, default = "/data/project/Meningioma/04.mutect/05.Shared_variant/241023_Dura.MT2.FMC.HF.RMBLACK.shared_variant.vcf")
parser.add_argument('--TUMOR_UNIQUE_VCF', type=str, default = "/data/project/Meningioma/04.mutect/06.Unique/241023_Dura.MT2.FMC.HF.RMBLACK.tumor_unique.vcf")
parser.add_argument('--OTHER_UNIQUE_VCF', type=str, default = "/data/project/Meningioma/04.mutect/06.Unique/241023_Dura.MT2.FMC.HF.RMBLACK.other_unique.vcf")
parser.add_argument('--THRESHOLD_END', type=int, default = 4)
parser.add_argument('--THRESHOLD_BQ', type=int, default = 10)
parser.add_argument('--THRESHOLD_MULTIALLELIC', type=int, default = 1)
parser.add_argument('--REMOVE_STR', type = str, default = "True")
parser.add_argument('--THRESHOLD_TUMOR_CLONAL_VAF', type=float, default = 0.2)
parser.add_argument('--MINIMUM_ALT', type=int, default = 1)
parser.add_argument('--REMOVE_CLIP', type=str, default = "False")
...
kwargs["TUMOR_BAM_PATH"] = args.TUMOR_BAM_PATH
kwargs["RESCUE_BAM_PATH"] = args.RESCUE_BAM_PATH
kwargs["BLOOD_BAM_PATH"] = args.BLOOD_BAM_PATH
...
kwargs ["THRESHOLD_END"], kwargs["THRESHOLD_BQ"], kwargs["THRESHOLD_MULTIALLELIC"] = args.THRESHOLD_END, args.THRESHOLD_BQ, args.THRESHOLD_MULTIALLELIC

kwargs["REMOVE_STR"] = args.REMOVE_STR
kwargs["THRESHOLD_TUMOR_CLONAL_VAF"] = args.THRESHOLD_TUMOR_CLONAL_VAF

kwargs["MINIMUM_ALT"] = args.MINIMUM_ALT


kwargs["REMOVE_CLIP"] = args.REMOVE_CLIP
for bool_key in ["REMOVE_STR", "REMOVE_CLIP"]:
    if kwargs[bool_key] == "True":
        kwargs[bool_key] = True
    elif kwargs[bool_key] == "False":
        kwargs[bool_key] = False
if kwargs["Sample_ID"].startswith("240110"):
    kwargs["REMOVE_CLIP"] = False
        ...
        for i, read in enumerate(iter):
            if read.is_proper_pair:
                ...
                if 'D' in cigar_string:
                    ...
                    if kwargs["REMOVE_CLIP"] == False:
                        if  (check ["End"] == 0) & (check["BQ"] == 0) & ( check ["Clustered_event"]  in [0, 1] ):   #통과해야만  ( ( check["Clip"] == 0 ) 는 줄지말지 애매하다)
                            base_counts [ read_base ] += 1
                            if reference_base != read_base:
                                if self.bamtype == "Dura":
                                    print ( "\t\tk = {}\tcurrent_pos = {}\treference_base = {}\tread_base = {}\tbase_quality = {}\tbase count({}) = {}".format (k, self.pos_, reference_base, read_base, base_quality, read_base, base_counts[read_base] ))
                    if kwargs["REMOVE_CLIP"] == True:
                        if  ( check["Clip"] == 0 )  & (check ["End"] == 0) & (check["BQ"] == 0) & ( check ["Clustered_event"]  in [0, 1] ):   #통과해야만  ( ( check["Clip"] == 0 ) 는 줄지말지 애매하다)
                            base_counts [ read_base ] += 1
                            if reference_base != read_base:
                                if self.bamtype == "Dura":
                                    print ( "\t\tk = {}\tcurrent_pos = {}\treference_base = {}\tread_base = {}\tbase_quality = {}\tbase count({}) = {}".format (k, self.pos_, reference_base, read_base, base_quality, read_base, base_counts[read_base] ))
...
if __name__ == "__main__":
    ...
    # tumor_shared_variant 파일에서 "#CHROM"으로 시작하는 라인을 찾아서 읽고, "Tumor"가 포함된 column이 몇 번째 column인지 출력하는 코드입니다.
    with open(kwargs["OTHER_SHARED_VARIANT_VCF"], "r") as f:
        for line in f:
            if line.startswith("#CHROM"):
                header = line.strip().split('\t')
                TUMOR_COL_INDEX = None
                for i in range(9, len(header)):
                    if "Blood" not in header[i]:
                        TUMOR_COL_INDEX = i
                        break
                print(f"TUMOR_COL_INDEX : {TUMOR_COL_INDEX}")
                break
    # other_shared_variant 파일에서 "#CHROM"으로 시작하는 라인을 찾아서 읽고, "Dura" or "Falx" or "Cortex" or "Ventricle"가 포함된 column이 몇 번째 column인지 출력하는 코드입니다.
    with open(kwargs["OTHER_SHARED_VARIANT_VCF"], "r") as f:
        for line in f:
            if line.startswith("#CHROM"):
                header = line.strip().split('\t')
                TISSUE_COL_INDEX = [i for i, col in enumerate(header) if kwargs["TISSUE"] in col][0]
                print(f'"TISSUE_COL)INDEX : {kwargs["TISSUE"]}"가 포함된 column index(zero-index): {TISSUE_COL_INDEX}')
                break
    print ("\n\n")
    ...
    for k in range (ti_df.shape[0]):
        ...
        if (len(ALT) == len(REF)) & (len(ALT) >= 2):  #이런건 지금 처리 못한다
            continue 
        Count_Tumor = Variant ( kwargs["Sample_ID"], "Tumor", kwargs["TUMOR_BAM_PATH"], CHR, END, 0, ALT_TYPE  )
        Count_Tumor.pysam_read ( **kwargs )
        Count_Blood = Variant ( kwargs["Sample_ID"], "Blood", kwargs["BLOOD_BAM_PATH"], CHR, END, 0, ALT_TYPE  )
        Count_Blood.pysam_read ( **kwargs )
        print ( "\n\n{}:{}\t{}>{}\t(Tumor VAF: {})".format ( CHR, END, REF, ALT, round ( Count_Tumor.base_counts [ Count_Tumor.alt_ ]  / Count_Tumor.depth_ , 2) ), end = "\n")
        Count = Variant ( kwargs["Sample_ID"], "Dura", kwargs["RESCUE_BAM_PATH"], CHR, END, 0, ALT_TYPE  )
        Count.pysam_read ( **kwargs )

    
        #2. Sample마다 MINIMUM_ALT 설정하기
        if ( kwargs["Sample_ID"] == "190524") | ( kwargs["Sample_ID"] == "230822")  | ( kwargs["Sample_ID"] == "230920")  | ( kwargs["Sample_ID"] == "231025")  | ( kwargs["Sample_ID"] == "230802_2") | ( kwargs["Sample_ID"] == "240412_2") :
            kwargs["MINIMUM_ALT"] = 1
        elif kwargs["Sample_ID"] == "220930_2":
            kwargs["MINIMUM_ALT"] = 5
        elif kwargs["Sample_ID"] == "240320":
            kwargs["MINIMUM_ALT"] = 3
        elif kwargs["Sample_ID"].startswith("230405"):
            kwargs["MINIMUM_ALT"] = 5
        else:
            if ( Count.depth_ >= 500 ) & ( CHR != "chr22" ):
                kwargs["MINIMUM_ALT"] = 2
            else:
                kwargs["MINIMUM_ALT"] = 1
        print ("\tMINIMUM_ALT = {}".format (kwargs["MINIMUM_ALT"]))


        #3. tumor mutect2 vcf에서 해당 부위 정보를 꺼내오기 (pyvcf)
        if (Count.base_counts [ Count.alt_ ] >= kwargs["MINIMUM_ALT"]) :        # Dura에서는 일정 숫자 이상이고, Blood에서는 아예 0일 것
            if Count_Tumor.depth_ < 10:
                print ("\tPass d/t tumor has depth count {} < 10".format (Count_Tumor.depth_))
                continue
            if ( Count_Tumor.base_counts [ Count_Tumor.alt_ ]  / Count_Tumor.depth_ < kwargs["THRESHOLD_TUMOR_CLONAL_VAF"]  ) :  # Tumor subclonal mutation이면 제외하자
                print ("\tPass d/t subclonal mutation in tumor {}/{} = {} < THRESHOLD_TUMOR_CLONAL_VAF {}".format (Count_Tumor.base_counts [Count_Tumor.alt_], Count_Tumor.depth_, round ( Count_Tumor.base_counts [Count_Tumor.alt_ ]  / Count_Tumor.depth_, 2) , kwargs["THRESHOLD_TUMOR_CLONAL_VAF"] ))
                print ("\t\tCount_Tumor\t{}\t{}".format ( Count_Tumor.base_counts, Count_Tumor.alt_ ) )
                continue

            vcf_reader_tumor_fetch = vcf_reader_tumor.fetch ( CHR, START, END )
            
            for tumor_line in vcf_reader_tumor_fetch:
                if kwargs["REMOVE_STR"] == True:
                    if "STR" in tumor_line.INFO.keys():   # Tumor VCF에서 STR==True라고 판명나면 그냥 버리기
                        if tumor_line.INFO["STR"] == True:
                            print ("\tRemove STR\t{}".format (tumor_line.INFO["STR"]))
                            continue

                # BAMSNAP 찍기 (Dura와 Tumor 한번에)
                print ( "\tIt is a shared mutation" )
                print ( "\t\t{} : {} / {}".format (kwargs ["TISSUE"], Count.base_counts [Count.alt_], Count.depth_ ) )
                print ( "\t\tTumor : {} / {}".format ( Count_Tumor.base_counts [Count_Tumor.alt_], Count_Tumor.depth_ ) )
                print ( "\t\tBlood : {} / {}".format (Count_Blood.base_counts [Count_Blood.alt_], Count_Blood.depth_ ) )
                if (Count_Blood.base_counts [Count_Blood.alt_] >= 1 ):
                    print ("\tPass d/t blood has alt count {}".format (Count_Blood.base_counts [Count_Blood.alt_]))
                    continue

                print ("\tBAMSNAP_IGV : {}".format ( kwargs["OTHER_SHARED_VARIANT_VCF"].replace ("vcf", str(CHR) + ":" + str(END) + ".jpg" ) ) )
                #BAMSNAP_IGV ( BAM_DIR_LIST = " ".join ( [ kwargs["RESCUE_BAM_PATH"], kwargs["RESCUE_BAM_PATH"].replace( kwargs["TISSUE"], "Tumor"), kwargs["BLOOD_BAM_PATH"] ] ), TITLE_LIST = str(CHR) + ":" + str(END), POS = str(CHR) + ":" + str(END), OUT = kwargs["OTHER_SHARED_VARIANT_VCF"].replace ("vcf", str(CHR) + ":" + str(END) + ".jpg") )
                ...
                # print other (Dura, Cortex, Ventricle) line : 기본적으로 Tumor에 있는 정보를 따라하지만, DP, AD. AF를 바꿔준다
                other_line = tumor_line
                new_call_data = vcf.model.make_calldata_tuple(["GT", "AD", "AF", "DP", "F1R2", "F2R1", "FAD", "SB"])(
                    GT = tumor_line.samples[TUMOR_COL_INDEX - 9].data.GT,
                    ...
                    DP = Count.depth_, 
                    F1R2 = tumor_line.samples[TUMOR_COL_INDEX - 9].data.F1R2,
                    F2R1 = tumor_line.samples[TUMOR_COL_INDEX - 9].data.F2R1,
                    FAD = tumor_line.samples[TUMOR_COL_INDEX - 9].data.FAD,
                    SB = tumor_line.samples[TUMOR_COL_INDEX - 9].data.SB
                )
                other_line.samples[ TISSUE_COL_INDEX - 9 ].data = new_call_data
                ...
                except:  # 아예 fetch가 안되는 것은  dura에서 해당 chromosome으로 된 contig가 없다는 얘기
                    print( "\t".join( [str(i) for i in print_rescue_line] ), file = other_rescue_vcf )
                    print( "\t".join( [str(i) for i in print_rescue_line] ), file = other_shared_variant )
