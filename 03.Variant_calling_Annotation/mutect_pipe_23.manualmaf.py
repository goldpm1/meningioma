...
while True:
    ...
    if line[0] == "#CHROM":
        for i in range(9, len(line)):
            ...
            ID_Alt.append([])
            ID_VAF.append([])
        control_index, control_samplename = FIND_CONTROL_SAMPLES ( ID_list, str_contain = "Blood" )
        #print ("Control samples: {} of {}th index".format ( control_samplename, control_index ))
        continue
    ...
    for j in range(len(VEP_verbose)):  # multiple transcript일 경우 하나씩 도는 것
        ...
        if (VEP_dict["BIOTYPE"] in ["protein_coding", "promoter", "CTCF_binding_site", "enhancer"]) | (VEP_dict["Consequence"] == "TF_binding_site_variant"):
            ...
            for i in range(9, len(line) ):        # Multisample일 수 있으니까
                if i - 9 == control_index:       # Blood는 제외하자
                    continue
...
# df_maf = df_maf.append(pd.DataFrame.from_records(matrix, columns=maf_colnames))  # 이건 deprecated
df_maf = pd.concat([df_maf, pd.DataFrame.from_records(matrix, columns=maf_colnames)], ignore_index=True)

# OUTPUT_MAF의 디렉토리가 존재하지 않으면 생성합니다.
import os
output_dir = os.path.dirname(OUTPUT_MAF)
