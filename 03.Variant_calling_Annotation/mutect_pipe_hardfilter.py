...
for line in input_file.readlines():
    ...
    if kwargs["REMOVE_MITOCHONDRIAL_DNA"] == True:
        ...
        if CHR not in ["chr" + str(i) for i in range (1,23) ] :
            check = 1        
    if kwargs["PASS"] == True:
    ...
    for i in range (9, len (line)):
        check2 = False
        if "Blood" in sample_name[i - 9]:
            continue
        if ( kwargs["SAMPLE_THRESHOLD"] == ["all"] ) | ( kwargs["SAMPLE_THRESHOLD"] == "all" ) :
            check2 = True
        else:
        ...
        if check2 == True:
            if int( sample_dict [i - 9] ["DP"] ) < kwargs["DP_THRESHOLD"]:
                print ("CHR = {}, POS = {}, REF = {}, ALT = {}, sample = {}  → DP = {}, AD = {}".format (CHR, POS, REF, ALT, sample_name[i - 9], sample_dict [i - 9] ["DP"], sample_dict [i - 9] ["AD"]))
                check = 1
                break
