...
for idx in ${!sample_name_LIST[@]}; do
    ...
    for TISSUE in Dura; do
        ...
        FINAL_BAM_PATH=${DATA_PATH}"/02.Align/"${hg}"/"${TISSUE}"/05.Final_bam/"${Sample_ID}"_"${TISSUE}".bam"
        DOC_PATH=${DATA_PATH}"/02.Align/"${hg}"/"${TISSUE}"/06.DepthOfCoverage/"${Sample_ID}"_"${TISSUE}".depth.result"
